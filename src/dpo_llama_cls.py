...
# Global Constants
DEFAULT_OUTPUT_DIR = "./out/cls-models"

def parse_arguments():
    """Parse command-line arguments."""
    parser = argparse.ArgumentParser(description="Run DPO training for paraphrase detection")
    parser.add_argument("--model_name", type=str, default="meta-llama/Llama-2-7b-hf", help="Path to the model")
    parser.add_argument("--adapter_dir", type=str, default="out/cls-models/llama-7b-qqp", help="Directory of the PEFT adapter")
...
def load_datasets(train_file, eval_file):
    ...
    logging.info(f"Loading datasets: {eval_file}")
    validation_dataset = load_dataset('json', data_files={'validation': eval_file})['validation']
    
    logging.info(f"Loading datasets: {train_file}")
...
def main():
    ...
    # Load datasets
    train_json_path = "out/detection_apty_ranked_train.jsonl"  # APTY training dataset
    eval_json_path = "out/detection_apty_ranked_test.jsonl"    # APTY evaluation dataset
